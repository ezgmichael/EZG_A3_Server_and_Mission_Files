...
buildingObjects = ["Land_CarService_F","Land_FuelStation_Shed_F","Land_fs_roof_F","Land_FuelStation_01_roof_F","Land_FuelStation_02_roof_F"]; //Define the buidlings that will act as triggers for the VMS in format ["building_Classname", "building_Classname 2"]
...
fuelingPrice = 0; //price of fuel per litre
vmsCostType = "Poptabs"; //Type of pricing
vmsSafeZoneCheck = 0; //Check that you are not in a safezone before enabling Service Centre and Vehicle Repair Options
...
vmsAggroCheck = 0; //Check that you are not aggro'd before enabling Service Centre and Vehicle Repair Options
vmsMarkers = 1; // Places a marker over each object that trigger vehicle service
vmsDisableStockRefuel = 0; // turn off or on the default refuelling option for the objects defined in vmsRefuelingObjects
