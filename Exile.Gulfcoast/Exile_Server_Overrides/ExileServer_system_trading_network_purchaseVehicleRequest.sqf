...
try 
{
	...
	_spawnDirection = getDir _nObject;
	_throwError = 0;
		...
		if (_disableCheck isEqualTo 1) then
		{
			switch (true) do {
				...
				case (_vehicleClass isKindOf "Air"): {
					_vehicleObject = [_vehicleClass, _position, _spawnDirection, true, _pinCode] call ExileServer_object_vehicle_createPersistentVehicle;
				};
				default {
					_vehicleObject = [_vehicleClass, _position, _spawnDirection, true, _pinCode] call ExileServer_object_vehicle_createPersistentVehicle;
				};
				...
				else
				{
					switch (true) do {
						...
						case (_vehicleClass isKindOf "Air"): {
							...
							{
								_vehicleObject = [_vehicleClass, _findEmpty, _spawnDirection, true, _pinCode] call ExileServer_object_vehicle_createPersistentVehicle;
							};
						};
						default {
							if (_disableRadius isEqualTo 1) then
