...
{
	{
		...
		if (_distance < (_x select 2)) exitWith
		{
			...
			else 
			{
			 ...
			 if !(["U_B_CBRN_Suit_01_MTP_F","U_B_CBRN_Suit_01_Tropic_F","U_B_CBRN_Suit_01_Wdl_F","U_I_CBRN_Suit_01_AAF_F","U_I_E_CBRN_Suit_01_EAF_F","U_C_CBRN_Suit_01_Blue_F","U_C_CBRN_Suit_01_White_F"] in (uniform player) || ["Exile_Headgear_GasMask","G_mas_usl_gasmask","G_mas_usl_gasmask_c"] in (assignedItems player) ) then
			{
            player setDamage ((damage player) + _damage);
			};
		};
